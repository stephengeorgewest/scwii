...
namespace noise
{
  ...
  namespace utils
  {
    ...
    // hope.)
    inline uint8* UnpackFloat (uint8* bytes, float value)
    {
      uint8* pBytes = (uint8*)(&value);
      bytes[0] = *pBytes++;
      bytes[1] = *pBytes++;
      bytes[2] = *pBytes++;
      bytes[3] = *pBytes++;
    ...
    inline uint8* UnpackLittle16 (uint8* bytes,
      noise::uint16 integer)
    {
      bytes[0] = (uint8)((integer & 0x00ff)      );
      bytes[1] = (uint8)((integer & 0xff00) >> 8 );
      return bytes;
    }

    // Unpacks a 32-bit integer value into four bytes in little endian format.
    inline uint8* UnpackLittle32 (uint8* bytes,
      noise::uint32 integer)
    {
      bytes[0] = (uint8)((integer & 0x000000ff)      );
      bytes[1] = (uint8)((integer & 0x0000ff00) >> 8 );
      bytes[2] = (uint8)((integer & 0x00ff0000) >> 16);
      bytes[3] = (uint8)((integer & 0xff000000) >> 24);
      return bytes;
    }
...
void WriterBMP::WriteDestFile ()
{
  ...
  try {
    pLineBuffer = new uint8[bufferSize];
  }
  catch (...) {
    throw noise::ExceptionOutOfMemory ();
  }
  ...
  const Color& backgroundColor, double lightValue) const
{
  ...
  // the new color.
  Color newColor (
    (uint8)((uint)(red   * 255.0) & 0xff),
    (uint8)((uint)(green * 255.0) & 0xff),
    (uint8)((uint)(blue  * 255.0) & 0xff),
  ...
  double bumpHeight) const
{
  ...
  // range.
  uint8 xc, yc, zc;
  xc = (noise::uint8)((uint)((floor)((vxc + 1.0) * 127.5)) & 0xff);
  yc = (uint8)((uint)((floor)((vyc + 1.0) * 127.5)) & 0xff);
  zc = (uint8)((uint)((floor)((vzc + 1.0) * 127.5)) & 0xff);
